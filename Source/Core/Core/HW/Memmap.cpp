...
bool MemoryManager::InitNCEGuestMapping()
{
  if (!m_is_initialized)
    return false;
  ...
  for (const PhysicalMemoryRegion& region : m_physical_regions)
  {
    ...
    const u32 addr = region.physical_address;
    const u32 size = region.size;

    // Map at physical address
    m_arena.MapInMemoryRegion(region.shm_position, size,
                              reinterpret_cast<void*>((uintptr_t)addr), true);

    // Map at the correct K1 cached alias (blocks 0x8–0xB).
    // The interpreter's GetSpanForAddress uses EA & 0x3FFFFFFF for K1 addresses,
    // which means K1 block B (0x8 + B) maps to physical offset (B * 0x10000000).
    // A region at physical_address P is only accessible through the K1 block
    // whose physical offset covers P, i.e. block = P >> 28.  Mapping at other
    // blocks would place the region data at different effective addresses than
    // what the interpreter reads, causing data consistency bugs (e.g., NCE
    // would fetch RAM data at 0x9xxxxxxx while the interpreter returns unmapped).
    // Only map when block matches the physical offset — skip blocks 1–3 for RAM.
    for (u32 block = 0; block < 4; block++)
    {
      const u32 kaddr = (0x80000000 + block * 0x10000000) | addr;
      if (kaddr != addr && block == (addr >> 28))
      {
        void* kresult = mmap(reinterpret_cast<void*>((uintptr_t)kaddr), size,
                             PROT_READ | PROT_WRITE | PROT_EXEC,
                             MAP_SHARED | MAP_FIXED, m_arena.GetSHMFD(),
                             region.shm_position);
        if (kresult == MAP_FAILED)
        {
          NOTICE_LOG_FMT(MEMMAP, "NCE K1 mmap failed at {} size {} errno={} {}",
                         fmt::ptr(reinterpret_cast<void*>((uintptr_t)kaddr)), size,
                         errno, strerror(errno));
        }
        else
        {
          NOTICE_LOG_FMT(MEMMAP, "NCE K1 mmap OK at {} size {} prot=RWX",
                         fmt::ptr(reinterpret_cast<void*>((uintptr_t)kaddr)), size);
        }
      }
    }

    // Map at the correct K1 uncached alias (blocks 0xC–0xF).
    // Same EA & 0x3FFFFFFF rule applies as for the cached alias.
    for (u32 block = 0; block < 4; block++)
    {
      const u32 uaddr = (0xC0000000 + block * 0x10000000) | addr;
      if (uaddr != addr && block == (addr >> 28))
      {
        void* uresult = mmap(reinterpret_cast<void*>((uintptr_t)uaddr), size,
                             PROT_READ | PROT_WRITE | PROT_EXEC,
                             MAP_SHARED | MAP_FIXED, m_arena.GetSHMFD(),
                             region.shm_position);
        if (uresult == MAP_FAILED)
        {
          NOTICE_LOG_FMT(MEMMAP, "NCE K1 uncached mmap failed at {} size {} errno={} {}",
                         fmt::ptr(reinterpret_cast<void*>((uintptr_t)uaddr)), size,
                         errno, strerror(errno));
        }
        else
        {
          NOTICE_LOG_FMT(MEMMAP, "NCE K1 uncached mmap OK at {} size {} prot=RWX",
                         fmt::ptr(reinterpret_cast<void*>((uintptr_t)uaddr)), size);
        }
      }
    }
  ...
  // L1 cache is accessed through the special >> 28 == 0xE path in MMU.cpp,
  // not through the GetSpanForAddress / EA & 0x3FFFFFFF path.  The per-region
  // loop above only maps at blocks matching addr >> 28, which for addr=0xE0000000
  // (>> 28 = 14) is outside the 0–3 range, so L1 cache would be unmapped under
  // NCE.  Map it explicitly at 0xE0000000 so that the special path works.
  for (const PhysicalMemoryRegion& region : m_physical_regions)
  {
    if (!region.active)
      continue;
    if (region.physical_address == 0xE0000000)
    {
      void* l1_result = mmap(reinterpret_cast<void*>((uintptr_t)0xE0000000ULL),
                             region.size, PROT_READ | PROT_WRITE | PROT_EXEC,
                             MAP_SHARED | MAP_FIXED, m_arena.GetSHMFD(),
                             region.shm_position);
      if (l1_result == MAP_FAILED)
      {
        NOTICE_LOG_FMT(MEMMAP, "NCE L1 mmap at 0xE0000000 failed, errno={} {}",
                       errno, strerror(errno));
      }
      else
      {
        NOTICE_LOG_FMT(MEMMAP, "NCE L1 mmap OK at 0xE0000000 size {} prot=RWX",
                       region.size);
      }
    }
  }

  // Map K2 segment (0x70000000-0x7FFFFFFF) as a RAM alias.
  // The Wii/GC IPL configures BATs so that K2 effective addresses map to
  // physical RAM (offset = effective & 0x0FFFFFFF).  The guest jumps into
  // this region for certain kernel/library code paths (e.g., at 0x78CE73C8).
  // Without this mapping every K2 access runs through the interpreter at
  // ~2 instructions/tick, making boot extremely slow.
  {
    const u32 k2_base = 0x70000000;
    const u32 k2_size = 0x10000000;  // 256 MB
    void* k2_result = mmap(reinterpret_cast<void*>((uintptr_t)k2_base), k2_size,
                           PROT_READ | PROT_WRITE | PROT_EXEC,
                           MAP_SHARED | MAP_FIXED, m_arena.GetSHMFD(), 0);
    if (k2_result == MAP_FAILED)
    {
      NOTICE_LOG_FMT(MEMMAP, "NCE K2 mmap failed at {} size {} errno={} {}",
                     fmt::ptr(reinterpret_cast<void*>((uintptr_t)k2_base)), k2_size, errno,
                     strerror(errno));
    }
    else
    {
      NOTICE_LOG_FMT(MEMMAP, "NCE K2 mmap OK at {} size {} prot=RWX",
                     fmt::ptr(reinterpret_cast<void*>((uintptr_t)k2_base)), k2_size);
    }
  }

  // Map K2 uncached segment (0xB0000000-0xBFFFFFFF) as another RAM alias.
  // Same physical offset as K2 cached (EA & 0x0FFFFFFF) but with caching disabled
  // on real hardware.  NCE doesn't simulate cache, so this is just another view of
  // the same SHM pages.  Guest code can branch here via K2 uncached function pointers
  // (e.g., Luigi's Mansion at 0xB7240470).
  {
    const u32 k2u_base = 0xB0000000;
    const u32 k2u_size = 0x10000000;  // 256 MB
    void* k2u_result = mmap(reinterpret_cast<void*>((uintptr_t)k2u_base), k2u_size,
                            PROT_READ | PROT_WRITE | PROT_EXEC,
                            MAP_SHARED | MAP_FIXED, m_arena.GetSHMFD(), 0);
    if (k2u_result == MAP_FAILED)
    {
      NOTICE_LOG_FMT(MEMMAP, "NCE K2 uncached mmap failed at {} size {} errno={} {}",
                     fmt::ptr(reinterpret_cast<void*>((uintptr_t)k2u_base)), k2u_size, errno,
                     strerror(errno));
    }
    else
    {
      NOTICE_LOG_FMT(MEMMAP, "NCE K2 uncached mmap OK at {} size {} prot=RWX",
                     fmt::ptr(reinterpret_cast<void*>((uintptr_t)k2u_base)), k2u_size);
    }
  }
...
void MemoryManager::ShutdownNCEGuestMapping()
{
  for (const PhysicalMemoryRegion& region : m_physical_regions)
  {
    if (!region.active)
    ...
    m_arena.UnmapFromMemoryRegion(reinterpret_cast<void*>((uintptr_t)addr), size);
    for (u32 block = 0; block < 4; block++)
    {
      if (block == (addr >> 28))
        m_arena.UnmapFromMemoryRegion(
            reinterpret_cast<void*>((uintptr_t)((0x80000000 + block * 0x10000000) | addr)), size);
    }
    for (u32 block = 0; block < 4; block++)
    {
      if (block == (addr >> 28))
        m_arena.UnmapFromMemoryRegion(
            reinterpret_cast<void*>((uintptr_t)((0xC0000000 + block * 0x10000000) | addr)), size);
    }
  }

  m_arena.UnmapFromMemoryRegion(reinterpret_cast<void*>((uintptr_t)0x70000000), 0x10000000);
  m_arena.UnmapFromMemoryRegion(reinterpret_cast<void*>((uintptr_t)0xB0000000), 0x10000000);

  // Unmap L1 cache at 0xE0000000 (mapped explicitly in InitNCEGuestMapping).
  for (const PhysicalMemoryRegion& region : m_physical_regions)
  {
    if (!region.active)
      continue;
    if (region.physical_address == 0xE0000000)
      m_arena.UnmapFromMemoryRegion(reinterpret_cast<void*>((uintptr_t)0xE0000000ULL),
                                    region.size);
  }
}
